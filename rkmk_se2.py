...
def constant_velocity(_, __):
    vec = np.array([0.01,0.0,0.0314])
    return vec

def time_var_velocity(_, t):
    vec = np.array([0.0314, 0.0, 0.03+0.05*np.sin(t/15.0)])
    return vec
...
def RKMK(steps, kinematics, sim_duration, butcher_tableu):
    ...
    for step in range(0,steps):
        ...
        for s in range(1,stages):
            ...
            theta = h*kinematics(T@SE2_ops.Exp(Theta), time+Cs[s]*h)
            inv_jac = SE2_ops.inv_right_jacobian(Theta)
            F = inv_jac@theta
            Fs.append(F)
        Theta = sum_to_current_est(Fs, Bs)
        T = T@SE2_ops.Exp(Theta)
        xs.append(T[0,2])
        ys.append(T[1,2])
        time += h
    plt.plot(xs,ys)
